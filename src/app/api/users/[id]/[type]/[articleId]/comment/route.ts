...
export async function POST(req: NextRequest, { params }: 
  { params: { id: string; type: string; articleId: string; isiKomentar:komentarTemplate } }) {
  const { id, type, articleId } = await params;
  const isiKomentar:komentarTemplate = await req.json();
  ...
  try {
    ...
    await collection.updateOne({ id }, { $set: type?{ sastra: user.sastra }:{article: user.article} })
